...
df = pd.read_csv("C:/Users/abhin/Downloads/RoadPy.csv")
...
if numeric_cols:
    ...
    sns.heatmap(corr, annot=True, fmt=".2f", cmap='YlOrRd', linewidths=0.5, square=True, cbar_kws={"shrink": .8})
...
def plot_actual_vs_pred_scatter(y_true, y_pred, model_name):
    ...
    plt.figure(figsize=(12,4))
    plt.scatter(x, y_true + np.random.uniform(-0.05, 0.05, len(x)), label="Actual", alpha=0.5, s=10)
    plt.scatter(x, y_pred + np.random.uniform(-0.05, 0.05, len(x)), label="Predicted", alpha=0.5, s=10)
...
def plot_counts(y_true, y_pred, model_name):
    ...
    df_plot = pd.DataFrame({"Actual": actual, "Predicted": pred})
